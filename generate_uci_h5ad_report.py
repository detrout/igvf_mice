...
DEFINITIONS = {
    "obs": {
        "Age_weeks": "integer representing the age of the mouse in weeks",
        "AnalysisSet": "IGVF accession for the intermediate analysis set this cell was originally called in",
        "analysis_set_accession": "IGVF accession for the intermediate analysis set this cell was originally called in",
        "Assay": "short name for the type of assay",
        "cellID": "Index of obs table. Includes barcoding wells for each round (1_2_3), subpool, and experiment.",
        "Donor": "IGVF accession for the donor mouse",
        "FileAccession": "IGVF file accession this for the intermediate analysis matrix that contained this cell",
        "Institution": "Name of the insittute that processed this bridge dataset",
        "lab_sample_id": "Human-readable sample name used in-house.",
        "MeasurementSet": "IGVF measurement set accession",
        "Mouse_ID": "UCI numeric ID representing a specific donor mouse",
        "MultiplexedSample": "IGVF MultiplexedSample accession if there were multiple donors in one measurement set, is nan/null if the measurement set was not multiplexed",
        "sample": "IGVF tissue accession.",
        "Old_file_accession": "Another copy of the IGVF file accession this for the intermediate analysis matrix that contained this cell",
        "plate": "Experiment ID, e.g. igvf_003.",
        "Preferred_Assay_Title": "a common name describing the specific type of experimental assay",
        "seqspec_region_id": "the region name from the corresponding seqspec that indicates which barcode to use for sample demultiplexing",
        "subpool": "Subpool ID, e.g. Subpool_2. Number corresponds to Illumina index ID from Parse Bio kit.",
        "Sample": "IGVF biosample accession for this cell",
        ...
        "barcode_seq": "a unique per cell barcode",
        "barcode": "24-nucleotide sequence corresponding to 3 8-nucleotide barcode rounds (3, 2, 1).",
        "bc": "24-nucleotide sequence corresponding to 3 8-nucleotide barcode rounds (3, 2, 1).",
        "bc1": "8-nucleotide sequence barcode for round 1.",
        "bc2": "8-nucleotide sequence barcode for round 2.",
        "bc3": "8-nucleotide sequence barcode for round 3.",
        ...
        "log1p_n_genes_by_counts": "The log1p of number of genes with at least 1 count.",
        "total_counts": "Total counts (UMIs), calculated using raw counts.",
        "total_counts_hb": "Total counts for hemoglobin genes, calculated using raw counts.",
        "total_counts_mt": "Total counts for mitochondrial genes, calculated using raw counts.",
        "total_counts_ribo": "Total counts for ribosomal genes, calculated using raw counts.",
        ...
        "log1p_total_counts": "The log1p of total counts (UMIs), calculated using raw counts.",
        "log1p_total_counts_hb": "log1p of total counts for hemoglobin genes, calculated using raw counts.",
        "log1p_total_counts_mt": "The log1p of total counts for mitochondrial genes, calculated using raw counts.",
        "log1p_total_counts_ribo": "The log1p of total counts for ribosomal genes, calculated using raw counts.",
        "pct_counts_hb": "Percent of total counts in hemoglobl genes, calculated using raw counts.",
        ...
        "pct_counts_ribo": "Percent of total counts in mitochondrial genes, calculated using raw counts.",
        "n_counts": "counts per cell before normalization", # From https://colab.research.google.com/github/buettnerlab/scanpy_demo/blob/main/tutorial_scanpy_basics.ipynb?hl=en#scrollTo=xP15EF0d12bx
        ...
        "parse_barcode_type": "which of the two parse biosciences barcode primer types this barocde is. A random primed (R) and poly T (T) barcode",
        ...
        "Subpool": "subpool is an internal name for the library being sequenced before it is submitted as a measurement set",
        "donor_accession": "IGVF donor accession this sample was derived from",
        "well": "Coordinate for the well the sample was initially loaded on before barcoding",
    ...
    "var": {
        "allele": "Which is the preferred allele",
        "ALT_allele": "True if the alternative allele the preferred allele",
        "B6J_allele": "True, if the B6J allele the preferred allele",
        "chromosome": "Which chromosome the gene is located on",
        "gene_name": "Human-readable gene name, e.g. Xkr4., may have a identifier to make unique appended",
        "gene_name_unique": "unmodified Human-readable gene name, e.g. Xkr4u",
        "gene_id": "ENSEMBL gene ID, e.g. ENSMUSG00000051951.6.",
        "gene_id_unmodified": "ENSEMBLE Gene ID without ALT allele name",
        "gene_name_unmodified": "Human-readable gene name without ALT allele name",
        "hb": "Boolean flag indicating of the gene is a hemoglobin gene",
        ...
        "log1p_total_counts": "log1p of total counts (UMIs), calculated using raw counts.",
        "log1p_mean_counts": "log1p of mean expression over all cells.",
    ...
    "uns": {
        "AnalysisSet_colors": "Hex color mappings for each source AnalysisSet",
        ...
        "Subpool_colors": "Hex color mapping by subpool (an internal name for the library being sequenced before it is submitted as a measurement set)",
        "umap": "Parameters used to compute the UMAP embedding.",
        "leiden": "leiden parameters",
        "lab_sample_id_colors": "Hex color mapping by lab sample id",
        ...
        "dendrogram_leiden": "object storing information needed to create endrogram plots of the leiden clustering",
